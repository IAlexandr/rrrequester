      ...
      (callback) => {
        ...
        async.eachLimit(kptResponses, 1, (response, done) => {
          ...
          if (features.length > 0) {
            async.eachLimit(features, 1000, (feature, cb) => {
              // TODO post
                ...
                superagent
                  ...
                  .end((err, rr) => {
                    if (err) {
                      l.log('error CN: ', CN);
                      setTimeout(() => {
                        l.log('try new rr CN: ', CN);
                        rSend(sendData);
                      }, 1000);
                      // return cb(err);
                    } else {
                      if (!rr || !rr.text) {
                        l.log('!!!! NO TEXT rr.text:', rr);
                      }
                      const rrrequest = JSON.parse(rr.text);
                      featureRequestsCount++;
                      infoMsg('featureRequestsCount:', featureRequestsCount, 1000);
